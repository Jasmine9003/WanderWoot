...
//add ip of frontend server and add proxy to frontend index.js
...
var getAllPropertiesWhere = (place, sleeps, email) => {
  return new Promise(function(resolve, reject) {
    pool.getConnection(function(err, con) {
      if (err) {
        console.log(err);
      } else {
        console.log("sweta test");
        console.log(place);
        console.log(email);
        var sql =
          "SELECT * FROM Property_Table WHERE (city = ? OR state = ? OR country = ?) AND accomodates>= ? AND uemail != ?";
        con.query(sql, [place, place, place, sleeps, email], function(
          err,
          rows
        ) {
          if (!err) {
        ...
        comparePasswords(password, rows[0].upasswordhash)
          .then(resp => {
            if (resp) {
              ...
              console.log("Logged in Sucessfully with " + email);
            } else {
              res.status(200).json({
                success: false,
...
app.post("/deleteProperty", function(req, res) {
  console.log("Property to delete :" + req.body.pid);
  pool.getConnection(function(err, con) {
    if (err) {
      res.writeHead(400, {
        "Content-Type": "text/plain"
      ...
      console.log("Going to delete the property: ", req.body.pid);
      con.query(
        "DELETE FROM property_table WHERE pid = ?",
        req.body.pid,
        (err, result) => {
          if (!err) {
...
app.post("/cancelBooking", function(req, res) {
  console.log("Booking to cancel :" + req.body.bid);
  pool.getConnection(function(err, con) {
    if (err) {
      res.writeHead(400, {
        "Content-Type": "text/plain"
      ...
      console.log("Cancelling Booking no : ", req.body.bid);
      con.query(
        "DELETE FROM booking_table WHERE bid = ?",
        req.body.bid,
        (err, result) => {
          if (!err) {
            console.log("Successfully cancelled booking.", req.body.bid);
            res.status(200).json({
              success: true
            });
          } else {
...
app.post("/getAllPropertiesWhere", function(req, res) {
  ...
  // console.log(email);
  getAllPropertiesWhere(place, people, email)
    .then(rows => {
      console.log(rows);
      let filteredRows = [];
      ...
      rows.forEach(row => {
        ...
        if (
          moment(endDate).diff(avf, "days") >= 0 &&
          moment(endDate).diff(avt, "days") <= 0 &&
          moment(startDate).diff(avf, "days") >= 0
        ) {
